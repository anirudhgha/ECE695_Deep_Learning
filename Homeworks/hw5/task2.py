...
A gaussian smoothing function is implemented and tested on datasets containing 4 
levels of noise. The gaussian smoothing function runs a convolution of the input
with a gaussian kernel of size 3x3. The gaussian kernel has a stride of 1, and a
padding layer is added so that the filter does not downsample the input image, 
allowing the rest of the network to function as it did before. 

The gaussian smoothing preprocessing layer increased the accuracy of the 50% noise
and 80% noise datasets significantly(by ~40% in both cases), but reduced the accuracy 
of the 0% noise and 20% noise datasets. These results are shown in output.txt. 
"""
...
#                    if self.dl_studio.debug_train and ((epoch==0 and (i==0 or i==9 or i==99)) or i%500==499):
        ...
        def run_code_for_testing_detection_and_localization(self, net, netnum):
 ...
 self.dataserver_train.class_labels[predicted[j]] for j in range(self.dl_studio.batch_size)))
    ...
    detector.dataserver_train = dataserver_train_0noise
    detector.dataserver_test = dataserver_test_0noise
    detector.load_PurdueShapes5_dataset(dataserver_train_0noise, dataserver_test_0noise)
    model_noisy_0 = detector.LOADnet2(skip_connections=True, depth=32)
    dls.show_network_summary(model_noisy_0)
    detector.run_code_for_training_with_CrossEntropy_and_MSE_Losses(model_noisy_0)
    #detector.run_code_for_training_with_CrossEntropy_and_BCE_Losses(model)
    detector.run_code_for_testing_detection_and_localization(model_noisy_0, netnum=1)
    
    # running model on noisy20
    detector.dataserver_train = dataserver_train_20noise
    detector.dataserver_test = dataserver_test_20noise
    detector.load_PurdueShapes5_dataset(dataserver_train_20noise, dataserver_test_20noise)
    model_noisy_20 = detector.LOADnet2(skip_connections=True, depth=32)
    dls.show_network_summary(model_noisy_20)
    detector.run_code_for_training_with_CrossEntropy_and_MSE_Losses(model_noisy_20)
    #detector.run_code_for_training_with_CrossEntropy_and_BCE_Losses(model)
    detector.run_code_for_testing_detection_and_localization(model_noisy_20, netnum=2)
    ...
    #detector.run_code_for_training_with_CrossEntropy_and_BCE_Losses(model)
    detector.run_code_for_testing_detection_and_localization(model_noisy_50, netnum=3)
    
    # running model on noisy80
    detector.dataserver_train = dataserver_train_80noise
    detector.dataserver_test = dataserver_test_80noise
    detector.load_PurdueShapes5_dataset(dataserver_train_80noise, dataserver_test_80noise)
    model_noisy_80 = detector.LOADnet2(skip_connections=True, depth=32)
    dls.show_network_summary(model_noisy_80)
    detector.run_code_for_training_with_CrossEntropy_and_MSE_Losses(model_noisy_80)
    #detector.run_code_for_training_with_CrossEntropy_and_BCE_Losses(model)
    detector.run_code_for_testing_detection_and_localization(model_noisy_80, netnum=4)
